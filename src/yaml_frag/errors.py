...
to catch :class:`YamlFragError`, print ``str(exc)`` to stderr, and exit with
``exc.exit_code``.
"""
...
class YamlFragError(Exception):
    """Base class for all yaml-frag errors.
...
class FragmentError(YamlFragError):
